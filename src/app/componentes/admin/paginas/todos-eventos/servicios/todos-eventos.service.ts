...
export class TodosEventosService {
  ...
  validarCruceFechas(filtro: Encuentro) {
    return this._http.get<Result<validarCruceFechasRes>>(
      `${environment.urlApi}ValidarFechasPartido?EquipoLocalId=${filtro.paisLocal}&EquipoVisitanteId=${filtro.paisVisitante}&FechaInicio=${filtro.fechaHoraInicio}&FechaFin=${filtro.fechaHoraFin}`
    );
  }
